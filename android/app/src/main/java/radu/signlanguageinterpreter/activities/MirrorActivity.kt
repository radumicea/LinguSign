...
import radu.signlanguageinterpreter.databinding.ActivityMirrorBinding
import radu.signlanguageinterpreter.fragments.MirrorFragment
...
class MirrorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.beginTransaction().replace(
            R.id.fragment_container, MirrorFragment()
        ).commit()

        onBackPressedDispatcher.addCallback(this) {
            startActivity(
                Intent(
                    this@MirrorActivity, HomeActivity::class.java
                )
            )
            finish()
            setResult(RESULT_OK)
